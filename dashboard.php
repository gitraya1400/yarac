...
// Logika untuk menangani form submission (tidak berubah)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    header('Content-Type: application/json');
    $response = ['success' => false, 'message' => 'An error occurred.'];
    $action = $_POST['action'];

    // ... (Logika untuk update_status, delete, save tetap sama persis seperti sebelumnya) ...
...
$stats_query = "SELECT (SELECT COUNT(*) FROM products) as total_products, (SELECT COUNT(*) FROM users WHERE role = 'user') as total_users, (SELECT COUNT(*) FROM orders) as total_orders, (SELECT SUM(total_amount) FROM orders WHERE status = 'delivered') as total_revenue, (SELECT COUNT(*) FROM users WHERE role = 'user' AND MONTH(created_at) = MONTH(CURDATE()) AND YEAR(created_at) = YEAR(CURDATE())) as new_users_monthly, (SELECT COUNT(*) FROM orders WHERE status = 'processing') as processing_orders";
$stats_stmt = $db->prepare($stats_query);
$stats_stmt->execute();
$stats = $stats_stmt->fetch(PDO::FETCH_ASSOC);
$average_order_value = ($stats['total_orders'] > 0 && $stats['total_revenue'] > 0) ? $stats['total_revenue'] / $stats['total_orders'] : 0;

// [PERUBAHAN 1] Tambahkan kolom 'notes' saat mengambil data pesanan
...
$low_stock_products = $db->query("SELECT id, name, category, stock FROM products WHERE stock < 20 ORDER BY stock ASC LIMIT 5")->fetchAll(PDO::FETCH_ASSOC);

// Data Lengkap untuk Tab Dinamis
...
// [PERUBAHAN 2] Tambahkan kolom 'notes' di sini juga
$orders_data = $db->query("SELECT o.id, o.total_amount, o.status, o.created_at, o.notes, u.first_name, u.last_name, u.email FROM orders o JOIN users u ON o.user_id = u.id ORDER BY o.created_at DESC")->fetchAll(PDO::FETCH_ASSOC);
$order_items_data = $db->query("SELECT oi.*, p.name as product_name FROM order_items oi JOIN products p ON oi.product_id = p.id")->fetchAll(PDO::FETCH_ASSOC);
...
<script>
    ...
    // Fungsi-fungsi JavaScript (openModal, closeModal, handleFormSubmit, deleteItem, dll) tetap sama
    // ...
    ...
    function viewOrderDetails(orderId) {
        ...
        // [PERUBAHAN 5] Tambahkan bagian untuk menampilkan notes di modal detail
        let notesHtml = '';
        if (order.notes && order.notes.trim() !== '') {
            notesHtml = `<hr><h4>Customer Notes:</h4><p style="white-space: pre-wrap;">${order.notes}</p>`;
        }

        const modalBody = `
            <h4>Customer: ${order.first_name} ${order.last_name} (${order.email})</h4>
            <p><strong>Total:</strong> Rp ${Number(order.total_amount).toLocaleString('id-ID')}</p>
    ...
    function loadContentForTab(type) {
        ...
        if(!data) { container.innerHTML = '<p>No data available.</p>'; return; }

        let headerHTML = '', bodyHTML = '';

        if (type === 'orders') {
            // [PERUBAHAN 6] Tambahkan header 'Notes' di tabel utama
            headerHTML = `<tr><th>Order ID</th><th>Customer</th><th>Total</th><th>Notes</th><th>Status</th><th>Actions</th></tr>`;
            bodyHTML = data.map(item => {
                // Siapkan notes untuk ditampilkan, potong jika terlalu panjang
                ...
                <tr>
                    ...
                    <td class="notes-cell" title="${item.notes || 'No notes'}">${notesSnippet}</td>
                    <td>
                        <select class="order-status-select" data-order-id="${item.id}" onchange="updateOrderStatus(this)">
                    ...
                    <td><button class="btn-action" onclick="viewOrderDetails(${item.id})">Details</button></td>
                </tr>`;
            }).join('');
        }
        // Logika untuk tab lain (products, advertisements, users) tidak berubah
            ...
            bodyHTML = data.map(item => `<tr><td><img src="assets/images/ads/${item.image || 'placeholder.jpg'}" class="table-image"></td><td>${item.title}</td><td>${item.sort_order}</td><td>${item.active == 1 ? 'Yes' : 'No'}</td><td><button class="btn-action" onclick="openModal('advertisement', ${item.id})">Edit</button> <button class="btn-action btn-delete" onclick="deleteItem(${item.id}, 'advertisement')">Delete</button></td></tr>`).join('');
             ...
             bodyHTML = data.map(item => `<tr><td>${item.first_name} ${item.last_name}</td><td>${item.email}</td><td>${item.role}</td><td>${new Date(item.created_at).toLocaleDateString('id-ID')}</td><td><button class="btn-action" onclick="viewUserHistory(${item.id})">History</button></td></tr>`).join('');
        }
        container.innerHTML = `<table class="admin-table"><thead>${headerHTML}</thead><tbody>${bodyHTML}</tbody></table>`;
    }

    // Kode JavaScript lainnya (DOMContentLoaded, logout, dll) tetap sama
    // ...
    // Fungsi-fungsi lain seperti openModal, closeModal, handleFormSubmit, deleteItem, updateOrderStatus, viewUserHistory, dll.
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        document.getElementById('product-image-upload').addEventListener('change', e => {
            ...
            if (file) {
                ...
                reader.onload = event => document.querySelector('#product-image-preview img').src = event.target.result;
        ...
        document.getElementById('ad-image-upload').addEventListener('change', e => {
            ...
            if (file) {
                const reader = new FileReader();
                reader.onload = event => document.querySelector('#ad-image-preview img').src = event.target.result;
                reader.readAsDataURL(file);
            }
        });
    ...
    async function updateOrderStatus(selectElement) {
        ...
        try {
            ...
            if (result.success) {
                ...
                location.reload(); // Reload untuk sinkronisasi jika gagal
            }
        } catch (error) {
    ...
    function viewUserHistory(userId) {
        ...
        if (!user) return;
        let ordersHtml = userOrders.length > 0 ? userOrders.map(order => `<div class="detail-item"><span>Order #${order.id} - ${new Date(order.created_at).toLocaleDateString('id-ID')}</span><span>Rp ${Number(order.total_amount).toLocaleString('id-ID')} <span class="status-badge status-${order.status}">${order.status}</span></span></div>`).join('') : '<p>This user has no order history.</p>';
        const modalBody = `<h4>User: ${user.first_name} ${user.last_name} (${user.email})</h4><hr><h4>Order History:</h4><div class="detail-list">${ordersHtml}</div>`;
        document.getElementById('detail-modal-title').innerText = `History for ${user.first_name}`;
        document.getElementById('detail-modal-body').innerHTML = modalBody;
        document.getElementById('detail-modal').style.display = 'block';
    }
...
<style>
/* CSS Tambahan untuk Kolom Notes */
.notes-cell {
    max-width: 200px;
    white-space: nowrap;
    overflow: hidden;
    text-overflow: ellipsis;
    cursor: help; /* Menunjukkan ada tooltip saat hover */
}
